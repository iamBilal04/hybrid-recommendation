...
const API_URL = 'https://ef6a-43-231-238-206.ngrok-free.app'
...
export const getProducts = async (activity?: number[]) => {
  try {
...
export const getProduct = async (productId: number) => {
  const response = await api.post('/store/show_product', { product_id: productId })
  return response.data
}

export const addToCart = async (productId: number) => {
  const response = await api.post('/store/add_to_cart', { product_id: productId })
  return response.data
}
...
export const getCheckoutRecommendations = async (activity: string) => {
  const response = await api.post('/store/checkout_recommendation', { activity })
